...
exports.recharge = async (req, res) => {
  try {
    ...
    // Directly update balance (instant recharge)
    ...
    const transaction = new FastagTransaction({
      ...
      description: 'FASTag wallet instant recharge'
    });
...
// Create Razorpay order for FASTag recharge
exports.createRazorpayOrder = async (req, res) => {
  try {
    const userId = req.user.id;
    const { amount, vehicleNumber } = req.body;

    console.log('Creating Razorpay order for user:', userId, 'amount:', amount, 'vehicle:', vehicleNumber);
    ...
    if (!vehicle) {
      console.error('No vehicle found for user:', userId, 'vehicleNumber:', vehicleNumber);
      return res.status(400).json({ message: 'No vehicle found for recharge. Please ensure you have a registered vehicle.' });
    }
    ...
    if (!order || !order.id) {
      console.error('Failed to create Razorpay order, response:', order);
...
exports.verifyRazorpayPayment = async (req, res) => {
  try {
    const userId = req.user.id;
    const { razorpay_payment_id, razorpay_order_id, razorpay_signature, amount, vehicleNumber } = req.body;

    if (!razorpay_payment_id || !razorpay_order_id || !razorpay_signature || !amount || !vehicleNumber) {
    ...
    if (!transaction) {
      console.error('Transaction not found for verification:', razorpay_order_id);
    ...
    console.log('Razorpay payment verified and processed successfully:', {
