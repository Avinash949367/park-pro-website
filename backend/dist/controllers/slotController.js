...
exports.createBooking = async (req, res) => {
    try {
        ...
        if (!['upi', 'card', 'cash', 'wallet', 'coupon'].includes(paymentMethod)) {
...
exports.reserveBooking = async (req, res) => {
    try {
        ...
        // Generate UPI payment payload using station's UPI ID if available
        const upiId = station.upiId || 'merchant@upi';
        const upiString = `upi://pay?pa=${upiId}&pn=${station.name}&am=${amount}&tn=BookingSlot&tr=${reservationId}`;

        res.status(201).json({
            reservation_id: reservationId,
            payment_payload: upiString,
            upi_id: upiId,
            payment_methods: station.paymentMethods || ['upi', 'card']
        });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
...
exports.createRazorpayOrder = async (req, res) => {
    try {
        ...
        // Check if user owns this booking
        if (!req.user || booking.userId.toString() !== req.user.id) {
            return res.status(403).json({ message: 'Access denied' });
        }
        ...
        console.log('Razorpay order created for booking:', {
            bookingId: bookingId,
            orderId: order.id,
            amount: order.amount,
...
exports.welcome = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Welcome to the Slot API!' });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
